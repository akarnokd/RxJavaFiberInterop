...
public class FiberInteropTest {
    ...
    public void virtualParent() {
        ...
        try (var exec = Executors.newSingleThreadExecutor()) {
            try (var scope = Executors.newThreadExecutor(Thread.builder().virtual(exec).factory())) {
    ...
    public void virtualParentScheduler() {
        ...
        try {
            var result = new AtomicReference<Boolean>();
            try (var scope = Executors.newThreadExecutor(Thread.builder().virtual(worker::schedule).factory())) {
                scope.submit(() -> result.set(Thread.currentThread().isVirtual()));
            }
    ...
    public void nestedVirtual() {
        ...
        try (var exec = Executors.newSingleThreadExecutor()) {
            try (var scope = Executors.newThreadExecutor(Thread.builder().virtual(exec).factory())) {
                try (var scope2  = Executors.newThreadExecutor(Thread.builder().virtual(scope).factory())) {
    ...
    static void withVirtual(Consumer<ExecutorService> call) throws Throwable {
        try (var exec = Executors.newThreadExecutor(Thread.builder().virtual().factory())) {
            call.accept(exec);
        }
    }

    static void withVirtual(Executor parent, Consumer<ExecutorService> call) throws Throwable {
        try (var exec = Executors.newThreadExecutor(Thread.builder().virtual(parent).factory())) {
            call.accept(exec);
        }
    }
}
